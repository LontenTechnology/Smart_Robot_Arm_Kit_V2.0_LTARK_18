...
void claw()
{
  ...
  if(x1>1000) //if push the left joystick to the left 
  {
    pos4=pos4+3; 
...
void turn()
{
    if(x2<50) //if push the right joystick to the let 
    {
      ...
      delay(5);
      if(pos1>180) //limit the angle when turns left 
      {
        pos1=180;
      }
    }
    ...
    {
      pos1=pos1-3; 
      myservo1.write(pos1); //the arm turns right. 
      delay(5);
      if(pos1<1) // limit the angle when turns right
      {
        pos1=1;
      }
    }
